...
parser.add_argument(
    "--top_n", type=int, default=1, help="number of paragraphs to be considered."
)
...
parser.add_argument(
    "--dtype", type=str, default=None, help="world size to use multiple GPUs."
)
parser.add_argument(
    "--world_size", type=int, default=1, help="world size to use multiple GPUs."
)
parser.add_argument(
    "--choices", type=str, default=None, help="space-separated answer candidates"
)
parser.add_argument("--instruction", type=str, default=None, help="task instructions")
parser.add_argument(
    "--download_dir", type=str, help="specify download dir", default=".cache"
)
...
def call_model_instructgpt(prompt, model, max_tokens=50):
    try:
        results = completions_instructgpt_backoff(
            model=model,
            prompt=prompt,
            temperature=0.0,
            max_tokens=max_tokens,
...
def main():
    isOpenAI = (
        True
        if args.model_name
        in ["text-davinci-003", "gpt-3.5-turbo-0301", "gpt-3.5-turbo"]
        else False
    )
    ...
    for idx in tqdm(range(len(input_data) // args.batch_size)):
        ...
        if isOpenAI is True:
            ...
            for input_instance in processed_batch:
                ...
                if (
                    args.model_name == "gpt-3.5-turbo-0301"
                    or args.model_name == "gpt-3.5-turbo"
                ):
